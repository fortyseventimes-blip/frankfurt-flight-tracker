...
function render(aircraft) {
  ...
  positionedAircraft.forEach(a => {
    ...
    el.innerHTML=`${aircraftIcon(type)}<span class="tag">${cleanFlight(a)}</span>`; el.setAttribute('aria-label',`Select aircraft ${cleanFlight(a)}`); el.addEventListener('click',event=>{ if(scalingGesture || performance.now()<suppressAircraftClickUntil){ event.preventDefault(); return; } selectAircraft(a,el); });
...
let zoomTimer; let suppressAircraftClickUntil=0; let scalingGesture=false; const pointerOrigins=new Map();
...
radar.addEventListener('wheel',event=>{ suppressAircraftClickUntil=performance.now()+350; event.preventDefault(); applyZoom(currentRange*(event.deltaY<0?.8:1.25)); },{passive:false});
radar.addEventListener('pointerdown',event=>{ pointers.set(event.pointerId,event); pointerOrigins.set(event.pointerId,{x:event.clientX,y:event.clientY}); radar.setPointerCapture(event.pointerId); if(pointers.size===2){scalingGesture=true; pinchStartDistance=pointerDistance(); pinchStartRange=currentRange;} });
radar.addEventListener('pointermove',event=>{ if(!pointers.has(event.pointerId)) return; pointers.set(event.pointerId,event); const origin=pointerOrigins.get(event.pointerId); if(origin && Math.hypot(event.clientX-origin.x,event.clientY-origin.y)>6) scalingGesture=true; if(pointers.size===2 && pinchStartDistance) applyZoom(pinchStartRange*pinchStartDistance/pointerDistance()); });
['pointerup','pointercancel','pointerleave'].forEach(type=>radar.addEventListener(type,event=>{ pointers.delete(event.pointerId); pointerOrigins.delete(event.pointerId); if(scalingGesture) suppressAircraftClickUntil=performance.now()+350; if(pointers.size<2){pinchStartDistance=0; pinchStartRange=currentRange;} if(pointers.size===0) scalingGesture=false; }));
